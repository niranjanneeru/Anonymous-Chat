...
from data import (active_chats, active_commands, delete_msg, asked_questionnaire, active_polls)
from database.db import Db
from utils import (MENU, OPTION_LIST, QUESTIONNAIRE_SET, ASK_FOR_QN)
...
def questionnaire(update: Update, context: CallbackContext):
    ...
    if Db.get_instance().get_questions(chat_id) is None:
        active_commands[chat_id] = context.bot.send_message(text=ASK_FOR_QN, chat_id=chat_id,
                                                            parse_mode=ParseMode.MARKDOWN).message_id
